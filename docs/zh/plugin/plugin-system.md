...
![模块依赖关系](../../assets/PIC/uml-module-relation.png)

总体可以分为 4 大部分：

- **第三方库**：Qt、SARibbon、pybind11 等基础依赖
- **DA 公共库**：DACommonWidgets、DAData、DAFigure 等通用模块
- **应用程序**：APP 主程序，提供基础框架
- **插件**：DataAnalysis 等功能插件，提供具体业务逻辑

插件、接口、主程序之间的调用关系：

![插件与接口关系](../../assets/PIC/uml-module-last3.png)
...
- [:material-code-block: 插件开发全流程](./plugin-development.md) - 从创建项目到完整实现
- [:material-sync: 插件生命周期](./plugin-lifecycle.md) - 生命周期管理详解
- [:material-folder: 插件模块详解](./plugin-module.md) - DAPluginSupport 模块