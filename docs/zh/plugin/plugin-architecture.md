...
DAWorkBench 采用插件化架构设计，将核心功能与扩展功能分离，通过标准接口实现模块间通信。本文档详细说明架构设计原则、接口规范、Python 脚本调用机制以及模块间通信协议。
...
- ✅ **Python 脚本集成**：业务逻辑可使用 Python 实现，利用 pandas 等数据处理库
...
- **开闭原则 (OCP)**：对扩展开放，对修改关闭，新增功能无需修改主程序
- **单一职责原则 (SRP)**：每个插件专注单一功能，界面与业务分离
- **依赖倒置原则 (DIP)**：高层不依赖低层，都依赖抽象接口
- **接口隔离原则 (ISP)**：接口职责单一，不强迫实现不需要的方法
...
理解模块间的依赖关系对于维护系统的稳定性至关重要。下图展示了 DAWorkBench 各模块之间的依赖关系，从第三方库到插件层的完整架构：
...
!!! note "Agent 工具注册插件模式"
    `DAAgent` 模块暴露 `DAAgentInterface` 契约，内置 agent 工具以插件形式注册：`plugins/DAAgentTools/` 在插件 `initialize()` 中调用 `DAAgentInterface::registerTool(DAAgentToolBase*)` 把 16 个内置工具注册进 `DAAgentModule` 的工具表。这与工作流节点插件（`DAAbstractNodePlugin` → `createNodeFactory()`）是两套并行的插件扩展点，前者面向 Agent 工具，后者面向工作流节点。详见 [Agent 开发指南](../dev-guide/agent/index.md)。
...
接口是插件化架构的核心，它们定义了主程序与插件之间的契约。良好的接口设计应该具备稳定性、清晰性和最小化特点。

DAWorkBench 定义了一组核心接口，下图展示了这些接口之间的层次结构关系：
...
- **DACoreInterface**：核心控制器接口，提供访问其他接口的入口
- **DAUIInterface**：界面操作接口，包含日志输出、对话框显示等功能
- **DADataManagerInterface**：数据管理接口，提供数据的增删改查功能
- **DAProjectInterface**：项目管理接口，提供项目保存、加载功能
- **DAStatusBarInterface**：状态栏接口，提供消息显示、进度条控制
- **DACommandInterface**：命令接口，提供撤销/重做功能
...
在应用程序启动过程中，各个接口的创建顺序必须严格遵循依赖关系。下图展示了正确的初始化顺序和各组件的创建时机：
...
1. 创建核心接口 DAAppCore
2. 初始化 Python 环境（必须在其他模块使用 Python 功能之前）
3. 创建数据管理器和项目管理器（不依赖 UI）
4. 创建主窗口（UI 组件容器）
5. 创建界面接口和命令接口
6. 完成初始化，应用开始运行
...
- **DAAbstractPlugin**：所有插件的抽象基类，包含指向 `DACoreInterface` 的指针
- **DAAbstractNodePlugin**：工作流节点插件的基类，继承自 `DAAbstractPlugin`
- **DAPluginManager**：插件管理器，单例模式，负责插件加载、卸载和生命周期管理
- **DataAnalysisPlugin**：具体插件示例，展示了数据分析插件的实现
...
理解插件的生命周期对于正确实现插件至关重要。下图展示了插件从加载到卸载的完整生命周期状态转换：
...
- **Unloaded**：插件尚未加载到内存
- **Loading**：正在加载动态库
- **Validating**：验证插件接口是否正确
- **Initializing**：调用 `initialize()` 方法初始化
- **Active**：插件已激活，准备就绪
- **Running**：插件正在执行功能
- **Retranslating**：处理语言变更
- **Error**：加载或初始化失败
- **Unloading**：正在卸载插件
...
DAWorkBench 支持在 C++ 代码中调用 Python 脚本，这使得业务逻辑可以用 Python 实现，充分利用 Python 丰富的数据处理库。下图展示了脚本调用的架构层次：
...
- **C++ 端**：包括主程序核心、插件和工作类，工作类封装 Python 调用
- **pybind11 绑定层**：将 C++ 接口暴露给 Python，包含三个模块：
  - `da_app`：应用级别的功能
  - `da_interface`：接口类型定义
  - `da_data`：数据类型定义
- **Python 脚本**：业务逻辑的实现，导入绑定模块访问主程序功能
...
下图展示了 C++ 调用 Python 函数的完整时序过程，包括 GIL 管理和用户交互：
...
上图展示了 Python 调用的标准流程：

1. Worker 类获取 GIL（全局解释器锁）
2. 导入 Python 模块并加载脚本
3. 调用 Python 函数执行业务逻辑
4. 如需用户输入，通过 `getConfigValues()` 显示对话框
5. Python 返回结果，Worker 类释放 GIL
...
1. Python 后台线程调用 `callInMainThread(callback)`
2. 信号处理器检查当前线程
3. 若不在主线程，包装回调函数并发射信号
4. Qt 主线程接收信号并执行回调函数
5. UI 操作在主线程中安全执行
...
数据管理器是模块间数据传递的核心组件。它提供了数据的存储、检索和通知功能。下图展示了数据从生产者到消费者的完整传递流程：
...
- **数据源**：Python 脚本、C++ 插件、用户导入将数据添加到管理器
- **数据管理器**：存储数据并通知监听者数据变更
- **数据消费者**：UI 显示、工作流节点、其他插件从管理器获取数据
...
事件通知是模块间通信的重要方式。DAWorkBench 提供了多种事件通知机制，包括项目状态变更、日志消息、进度报告等。下图展示了典型的事件通知交互过程：
...
1. 插件调用 `setProjectDirty(true)` 通知项目已修改
2. UI 接口更新窗口标题显示未保存状态
3. 插件调用 `addInfoLogMessage()` 输出日志并显示在状态栏
4. 长时间操作时显示进度条并定期更新进度
5. 调用 `processEvents()` 保持界面响应
...
以下是一个完整的数据分析插件工作流程，展示了从用户点击按钮到操作完成的全过程。下图演示了各组件之间的交互顺序：
...
1. 用户点击"删除缺失值"按钮，触发插件 Action
2. 插件调用 Worker 类的 `dropna()` 方法
3. Worker 类获取 GIL 并导入 Python 模块
4. Python 脚本从数据管理器获取选中数据
5. 显示配置对话框获取用户参数
6. 使用 pandas 处理数据并写回数据管理器
7. 触发 UI 更新并返回结果
...
- [插件与接口说明](./plugin-system.md)
- [插件模块详解](./plugin-module.md)
- [接口模块详解](../dev-guide/architecture/interface-module.md)
- [C++ 与 Python 集成](../dev-guide/python-binding/index.md)
