...
图表窗口在 `DAChartOperateWidget` 中以 ADS 嵌套停靠区管理（多窗口自由分屏、隔离在绘图区内），其设计与一个 `FocusHighlighting` 焦点跨管理器陷阱详见 [绘图窗口停靠布局](../graphics/chart-dock-nesting.md)。
...
详细步骤参见 [Python 节点开发指南](../workflow/workflow-python-node-dev.md)。
...
详细步骤参见 [插件项目创建指南](../../plugin/plugin-development.md)。
...
详细步骤参见 [创建设置面板指南](../ui/creating-setting-panel.md)。
