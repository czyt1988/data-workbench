...
下图展示了工作流核心类之间的继承和组合关系，帮助理解模块的整体架构设计：

- **DAWorkFlow** 是工作流的核心容器类，管理所有节点和工厂
- **DAAbstractNodeFactory** 是节点工厂的抽象基类，负责创建特定类型的节点
- **DAAbstractNode** 是节点的抽象基类，定义节点的执行和连接接口
- **DANodeMetaData** 存储节点的元数据信息，用于节点创建和显示
- **DAWorkFlowExecuter** 是工作流执行器，在独立线程中执行工作流
...
上图通过类图展示了工作流模块的核心架构，其中：

- `DAWorkFlow` 使用组合关系管理多个 `DAAbstractNode` 节点实例
- `DAWorkFlow` 通过注册机制关联多个 `DAAbstractNodeFactory` 工厂
- `DAAbstractNodeFactory` 创建 `DAAbstractNode` 节点实例
- `DAAbstractNode` 持有 `DANodeMetaData` 元数据对象
- `DAWorkFlow` 使用 `DAWorkFlowExecuter` 执行工作流
...
下图展示了通过元数据创建节点的完整时序过程，描述了用户、工作流、工厂和节点之间的交互顺序：
...
1. 用户调用 `createNode(metaData)` 请求创建节点
2. 工作流根据原型查找对应的工厂实例
3. 工作流调用工厂的 `create(metaData)` 方法
4. 工厂创建节点实例并调用 `initializNode(node)` 进行初始化
5. 工作流为节点分配唯一 ID 并记录管理信息
6. 最终返回节点的智能指针给用户

### 工作流执行时序图

下图展示了工作流执行的完整时序过程，描述了从用户触发执行到完成的全流程：
...
1. 用户设置起始节点并调用 `exec()` 触发执行
2. 工作流创建执行器并移动到独立线程
3. 执行器按依赖顺序循环执行各节点
4. 每个节点执行完成后，通过信号通知工作流和用户
5. 所有节点执行完成后，发射 `finished` 信号通知用户
