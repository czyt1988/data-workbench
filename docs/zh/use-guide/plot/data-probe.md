...
本文档介绍数据探针功能的使用方法，数据探针是图表中标注和捕获特定位置数据的交互工具。
...
探针自动追踪所有可见曲线，计算并存储相交点数据。下图展示了探针追踪曲线的完整过程：
...
1. **探针位置确定**：用户设置探针的 X 值（垂直探针）或 Y 值（水平探针）
2. **遍历所有曲线**：依次检查图表中的每条可见曲线
3. **判断穿过条件**：检查曲线数据范围是否包含探针值
4. **计算相交点**：使用直接取值或线性插值计算精确坐标
5. **存储捕获数据**：将捕获结果存入数据列表
6. **显示捕获结果**：在探针数据表格中展示所有捕获点
