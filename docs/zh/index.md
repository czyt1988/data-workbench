...
| [:material-folder: 项目结构](./structure/project-structure.md) | 目录组织和模块职责 |
...
| [:material-puzzle: 插件开发指南](./plugin/plugin-development.md) | 开始开发自己的插件 |
...
DAWorkBench 采用五层架构（基础层→功能层→界面层→接口层→应用层）。其中 `src/DAAgent/` 为 Agent 子系统所在模块，位于接口层（L4），对外暴露 `DAAgentInterface`，供插件注册工具、配置 LLM、管理会话与提示词库；其运行时由独立 Python 子进程（LangGraph）承载。详见 [API 总览](./reference/api-reference/overview.md)。
...
- [:material-book: 项目结构详解](./structure/project-structure.md) - 了解项目目录和模块组织
