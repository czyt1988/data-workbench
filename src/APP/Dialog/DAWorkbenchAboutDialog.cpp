...
{
	ui->textEdit->append(
		tr("DAWorkBench,LGPL, Version:%1.%2.%3").arg(DA_VERSION_MAJ).arg(DA_VERSION_MIN).arg(DA_VERSION_PAR));
	...
	ui->textEdit->append(tr("SARibbon, MIT, Version:%1.%2.%3")
							 .arg(SA_RIBBON_BAR_VERSION_MAJ)
							 .arg(SA_RIBBON_BAR_VERSION_MIN)
							 .arg(SA_RIBBON_BAR_VERSION_PAT));
	ui->textEdit->append(
		tr("spdlog, MIT, Version:%1.%2.%3").arg(SPDLOG_VER_MAJOR).arg(SPDLOG_VER_MINOR).arg(SPDLOG_VER_PATCH));
	...
	ui->textEdit->append(tr("pybind11,BSD,Version:%1.%2").arg(PYBIND11_VERSION_MAJOR).arg(PYBIND11_VERSION_MINOR));  // cn:这是依赖python的版本
#endif
}
}  // end DA
