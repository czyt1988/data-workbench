...
public:
    ...
    {
    public:
        using FunPtr = std::function< void(DAAppArchive*) >;
        Task();
        Task(const QString& path, const QByteArray& d, const QString& des = QString());
        Task(const QString& path, const FunPtr& fp, const QString& des = QString());
        ~Task();
...
Q_SIGNALS:
     ...
     * @param pos 当前任务的位置
     */
    void taskProgress(int total, int pos, const QString& info);
