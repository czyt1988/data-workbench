...
void DAAgentSettingsWidget::setupUI()
{
    ...
    m_testBtn     = new QPushButton(tr("Test Connection"), this);  //cn:测试连接
...
void DAAgentSettingsWidget::onTestConnection()
{
    ...
    m_statusLabel->setText(tr("Testing..."));  //cn:测试中...
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, model]() {
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            m_statusLabel->setText(tr("✓ Connected (%1)").arg(modelUsed));  //cn:✓ 连接成功 (%1)
        } else {
            m_statusLabel->setStyleSheet("color: red;");
            m_statusLabel->setText(tr("✗ Connection failed: %1").arg(reply->errorString()));  //cn:✗ 连接失败: %1
        }
        reply->deleteLater();
    });
}
