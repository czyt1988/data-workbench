 ...
 * @param pos 当前位置
 */
void DAAppProject::onTaskProgress(int total, int pos)
{
}

/**
 * @brief DAAppProject::saveTaskFinish
 * @param code
 */
void DAAppProject::onSaveTaskFinish(int code)
{
    ...
    if (code == DAAppArchive::SaveSuccess) {
        qInfo() << tr("Success save archive to %1").arg(savePath);
    } else {
        qWarning() << tr("Failed to save archive %1").arg(savePath);
    }
}
