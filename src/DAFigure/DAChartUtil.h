#ifndef DACHARTUTIL_H
#define DACHARTUTIL_H
...
///
class DAFIGURE_API DAChartUtil
{
public:
    // 获取一个默认的绘图名称
    static QString plotTitle(QwtPlot* plot, QwtFigure* fig);
    // QwtPlotItem的名字
    static QString plotItemName(QwtPlotItem* item);
    // QwtPlotItem的图标
    static QIcon plotItemIcon(QwtPlotItem* item);
...
template< typename T >
void DAChartUtil::getSeriesData(QVector< T >& vec, const QwtSeriesStore< T >* series, int startIndex, int endIndex)
{
...
template< typename T, typename PlotItemType >
void DAChartUtil::setVectorSampleData(QwtPlotItem* item, const QVector< T >& datas)
{
