...
void DAChartWidget::removeCurve(QwtPlotCurve* curve)
{
    ...
    emit plotItemRemoved(curve);
    emit dataChanged();
}

QwtPlotCurve* DAChartWidget::addScatter(const QVector< double >& xData, const QVector< double >& yData, const QString& title)
{
    QwtPlotCurve* curve = addCurve(xData, yData, title);
    if (curve) {
                                                      ...
                                                      const QString& title)
{
    ...
    emit plotItemAdded(intervalCurve);
    emit dataChanged();
...
QwtPlotMarker* DAChartWidget::addCrossLine(double x, double y, const QString& title)
{
    ...
    marker->setYValue(y);
    marker->setLineStyle(QwtPlotMarker::Cross);
    marker->setLinePen(QPen(Qt::darkGray, 1, Qt::DotLine));
    marker->attach(this);
...
QwtPlotSpectrogram* DAChartWidget::addSpectrogram(QwtGridRasterData* gridData, const QString& title)
{
    ...
    emit plotItemAdded(spectrogram);
    emit dataChanged();

    return spectrogram;
}

QList< QwtPlotItem* > DAChartWidget::getAllPlotItems() const
{
    return itemList();
}
...
void DAChartWidget::clearAllData()
{
    ...
    for (QwtPlotItem* item : items) {
        if (item->rtti() == QwtPlotItem::Rtti_PlotCurve || item->rtti() == QwtPlotItem::Rtti_PlotBarChart
            || item->rtti() == QwtPlotItem::Rtti_PlotIntervalCurve || item->rtti() == QwtPlotItem::Rtti_PlotMarker
            || item->rtti() == QwtPlotItem::Rtti_PlotSpectrogram) {
            item->detach();
            delete item;
        }
    }
...
bool DAChartWidget::hasData() const
{
    ...
    for (const QwtPlotItem* item : items) {
        if (item->rtti() == QwtPlotItem::Rtti_PlotCurve || item->rtti() == QwtPlotItem::Rtti_PlotBarChart
            || item->rtti() == QwtPlotItem::Rtti_PlotIntervalCurve || item->rtti() == QwtPlotItem::Rtti_PlotSpectrogram) {
            return true;
        }
    }
    return false;
...
void DAChartWidget::setCanvasBackground(const QBrush& brush)
{
    d_ptr->canvasBackground = brush;
    setCanvasBackground(brush.color());  // QwtPlot 的方法
...
QBrush DAChartWidget::getCanvasBackground() const
{
    return d_ptr->canvasBackground;
}
...
void DAChartWidget::setAxisFont(int axisId, const QFont& font)
{
    setAxisFont(axisId, font);
    notifyPropertiesChanged();
}

QFont DAChartWidget::getAxisFont(int axisId) const
{
...
{
    QwtScaleWidget* axisWidget = this->axisWidget(axisId);
    if (axisWidget) {
...
{
    QwtScaleWidget* axisWidget = this->axisWidget(axisId);
    return axisWidget ? axisWidget->palette().color(QPalette::Text) : QColor();
}

void DAChartWidget::showAxis(int axisId, bool visible)
{
    enableAxis(axisId, visible);
    notifyPropertiesChanged();
}

bool DAChartWidget::isAxisVisible(int axisId) const
{
    return axisEnabled(axisId);
}
...
{
    if (d_ptr->legend) {
        // QwtPlotLegendItem 的位置设置可能需要特殊处理
        // 这里简化为重新创建图例
        bool wasVisible = d_ptr->legend->isVisible();
        d_ptr->legend->detach();
        delete d_ptr->legend;
        d_ptr->legend = nullptr;

        enableLegend(wasVisible);
...
{
    return dynamic_cast< QwtDateScaleDraw* >(axisScaleDraw(axisId)) != nullptr;
}

void DAChartWidget::setAxesMargin(int margin)
{
    if (d_ptr->axesMargin != margin) {
        d_ptr->axesMargin = margin;
...
int DAChartWidget::getAxesMargin() const
{
    return d_ptr->axesMargin;
}

void DAChartWidget::setAutoReplot(bool autoReplot)
{
    if (d_ptr->autoReplot != autoReplot) {
        d_ptr->autoReplot = autoReplot;
        QwtPlot::setAutoReplot(autoReplot);
        notifyPropertiesChanged();
    }
}

bool DAChartWidget::isAutoReplot() const
{
    return d_ptr->autoReplot;
}
...
void DAChartWidget::zoomToRect(const QRectF& rect)
{
    if (rect.isValid()) {
...
void DAChartWidget::enableCrosshair(bool enable)
{
    ...
    if (d_ptr->crosshair) {
        ...
        emit crosshairStateChanged(enable);
        notifyPropertiesChanged();
    }
}
...
void DAChartWidget::enableDataPicking(bool enable)
{
    enableYValuePicking(enable);
    enableXYValuePicking(enable);
}
...
void DAChartWidget::enableYValuePicking(bool enable)
{
    ...
    if (d_ptr->dataPicker) {
        ...
        emit dataPickingStateChanged(isDataPickingEnabled());
        notifyPropertiesChanged();
    }
}
...
void DAChartWidget::enableAllInteractions(bool enable)
{
    enableZoom(enable);
    enablePan(enable);
    enableCrosshair(enable);
    enableDataPicking(enable);
    enableMouseWheelZoom(enable);
    enableLegendPanel(enable);
}

void DAChartWidget::disableAllInteractions()
{
...
void DAChartWidget::refresh()
{
    replot();
}

void DAChartWidget::notifyPropertiesChanged()
{
    emit chartPropertiesChanged(this);
}
...
void DAChartWidget::setupCrosshair()
{
    ...
    if (d_ptr->pickerFactory) {
        ...
        d_ptr->crosshair->setTrackerMode(QwtPicker::ActiveOnly);
