...
namespace DA
{
DAZipArchiveTask_ChartItem::DAZipArchiveTask_ChartItem(const DAChartItemsManager& items)
    : DAAbstractArchiveTask(), mItems(items)
{
}
...
bool DAZipArchiveTask_ChartItem::writeChartItems(DAZipArchive* zip, const DAChartItemsManager* itemsData)
{
    ...
    if (!zip->write(QStringLiteral("chart-data/chart-items.xml"), itemsInfoXmlData)) {
        qDebug() << QString("writechart-data/chart-items.xml error");
        return false;
    }
    //! 2.写绘图的数据到文件中
