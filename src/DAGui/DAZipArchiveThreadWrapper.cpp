...
void DAZipArchiveThreadWrapper::init()
{
    ...
    connect(archive, &DAAbstractArchive::taskProgress, this, &DAZipArchiveThreadWrapper::onTaskProgress);
    // 信号绑定触发saveall
 ...
 * @brief 任务进度
 *
 * 这个任务进度主要处理load的回调，task是在线程中执行，所以不能在任务中处理回调，回调放在wrapper里执行，这样可以在回调里操作ui
 *
 * 执行完回调后，才会再发出@ref taskProgress 信号
 * @param task 任务
 * @param mode 模式
 */
void DA::DAZipArchiveThreadWrapper::onTaskProgress(std::shared_ptr< DAAbstractArchiveTask > task, int mode)
{
