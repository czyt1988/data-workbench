                                           ...
                                           std::optional< int > thresh)
{
    ...
    std::unique_ptr< DACommandDataFrame_dropna > cmd =
        std::make_unique< DACommandDataFrame_dropna >(df, axis, how, index, thresh);
    DADataTableModel* modle = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
...
{
    std::unique_ptr< DACommandDataFrame_fillna > cmd = std::make_unique< DACommandDataFrame_fillna >(df, value, limit);
    DADataTableModel* modle                          = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
...
bool DADataOperateOfDataFrameWidget::interpolate(const DAPyDataFrame& df, const QString& method, int order, int limit)
{
    std::unique_ptr< DACommandDataFrame_interpolate > cmd =
        std::make_unique< DACommandDataFrame_interpolate >(df, method, order, limit);
    DADataTableModel* modle = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
...
{
    std::unique_ptr< DACommandDataFrame_ffillna > cmd = std::make_unique< DACommandDataFrame_ffillna >(df, axis, limit);
    DADataTableModel* modle                           = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
...
{
    std::unique_ptr< DACommandDataFrame_bfillna > cmd = std::make_unique< DACommandDataFrame_bfillna >(df, axis, limit);
    DADataTableModel* modle                           = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
...
int DADataOperateOfDataFrameWidget::dropduplicates(const DAPyDataFrame& df, const QString& keep, const QList< int > index)
{
    std::unique_ptr< DACommandDataFrame_dropduplicates > cmd =
        std::make_unique< DACommandDataFrame_dropduplicates >(df, keep, index);
    DADataTableModel* modle = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
...
int DADataOperateOfDataFrameWidget::nstdfilteroutlier(const DAPyDataFrame& df, double n, int axis, const QList< int > index)
{
    std::unique_ptr< DACommandDataFrame_nstdfilteroutlier > cmd =
        std::make_unique< DACommandDataFrame_nstdfilteroutlier >(df, n, axis, index);
    DADataTableModel* modle = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
...
bool DADataOperateOfDataFrameWidget::clipoutlier(const DAPyDataFrame& df, double lower, double upper, int axis)
{
    std::unique_ptr< DACommandDataFrame_clipoutlier > cmd =
        std::make_unique< DACommandDataFrame_clipoutlier >(df, lower, upper, axis);
    DADataTableModel* modle = mModel;
    cmd->setCallBack([ modle ]() {
        if (modle) {
            modle->refreshData();
