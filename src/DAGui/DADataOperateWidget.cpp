 ...
 * 信号，运行期新建的数据表停靠区（含拖拽拆分、布局恢复）自动应用该方位。
 * @param atBottom true=底部，false=顶部
 */
void DADataOperateWidget::setInnerDockTabsAtBottom(bool atBottom)
{
    DA::DADockAreaTabPosition::applyToDockManager(d_ptr->_dockManager, atBottom);
}
