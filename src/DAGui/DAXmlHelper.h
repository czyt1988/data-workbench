...
public:
    ...
    // 创建剪切板描述xml
    QDomElement makeClipBoardElement(
        const QList< DAGraphicsItem* > its, const QString& tagName, QDomDocument* doc, bool isCopyType = true
    );
    ...
    static bool loadElement(
        DAGraphicsScene* scene,
    ...
    // DAFigureWidget
    static QDomElement makeElement(
        const DAFigureWidget* fig, const QString& tagName, QDomDocument* doc, DAChartItemsManager* itemsMgr
    );
    static bool loadElement(
        DAFigureWidget* fig, const QDomElement* tag, const DAChartItemsManager* itemsMgr, const QVersionNumber& v = QVersionNumber()
    );
    // DAChartWidget
    static QDomElement makeElement(
    ...
    static bool loadQwtPlotAxisElement(
        DAChartWidget* chart, const QDomElement* qwtplotTag, const QVersionNumber& v = QVersionNumber()
    );
    // QwtPlotLayout 相关
