...
class DAPlotTreeModel;

/**
 * @brief 绘图树
 */
class DAGUI_API DAFigureTreeView : public QTreeView
{
    Q_OBJECT
...
public:
    ...
    ~DAFigureTreeView();
    // 设置figure
    void setFigureWidget(DAFigureScrollArea* fig);
    DAFigureScrollArea* getFigureWidget() const;
    // 刷新
    void refresh();
    // 获取内部模型
    DAPlotTreeModel* getFigureTreeModel() const;
    // 设置自动适应内容
...
Q_SIGNALS:
    // 请求改变图元颜色的信号
    void requestItemChangeColor(QIM::QImPlotItemNode* item, QStandardItem* treeItem);
