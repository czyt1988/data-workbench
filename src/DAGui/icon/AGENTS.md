# DAGui 图标设计风格指引（`src/DAGui/icon/`）

> **用途**：本目录存放 DAWorkbench 主界面（DAGui 模块）所用的 SVG 图标资源。本文档用于指导 AI（或设计师）在新增、替换图标时保持视觉一致性。
>
> **阅读对象**：AI Agent、UI 设计 contributor。
...
- **语义化配色**：颜色本身传递含义（绿=确认、红=删除、蓝=主操作、黄=收藏），详见 §3。
- **无文字**：图标中不嵌入任何文字字符（数据类型图标中的 `I/F/T/O` 等是字形图形化绘制的，不是 `<text>` 元素）。
- **边缘留白少**：图标的具体内容和四个边缘留白不能太多，可以有一点间隙，这样能在工具栏中尽量占满,例如`32*32`的图标，绘制区域应该在`30*30`或`28*28`范围,左右可以和边界留1~2的间隙
---
...
> **关键**：颜色不是随意的，每种颜色都承担语义。新增图标必须按语义选色，不要引入新的色值。

### 3.1 主色系 — 蓝（主操作 / 数据 / 链接）

| 色值 | 角色 | 典型用途 |
|------|------|---------|
| `#5280C1` | **主蓝（Primary Blue）** | 主操作轮廓、强调元素、数据类型卡片边框 |
| `#497CAD` | **深蓝（Secondary Blue）** | iconfont 类单色图标、链接、工作流连线、轴向标记 |
| `#BDCDE8` | **浅蓝（Light Blue Fill）** | 容器背景、次级填充、卡片底色 |
...
| 色值 | 角色 | 典型用途 |
|------|------|---------|
| `#727272` | **中灰（Primary Gray）** | 图标主轮廓线、中性容器边框、未激活元素 |
| `#515151` | **深灰** | 文字图形（如 `data.svg` 表头）、强调轮廓 |
| `#B2B2B2` / `#BFBFBF` / `#7F7F7F` | **浅灰系** | 次要轮廓、禁用态、副图层 |
...
> 这套配色来自 iconfont 第三方图标库，是本目录中唯一的「非主色系」特例。新增消息类型图标请沿用此套色系。
...
- ❌ 引入动画（`<animate>`、`<animateTransform>` 等）—— 本目录所有图标均为静态
