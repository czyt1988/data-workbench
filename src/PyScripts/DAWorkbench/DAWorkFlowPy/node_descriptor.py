...
- outputs: 输出端口列表
- parameters: 参数列表
- render_template: 渲染模板类型（'rect'、'svg'、'widget'）
"""
...
class DANodeDescriptor:
    ...
    :param render_template: 渲染模板类型，默认为 'rect'
    """
    ...
    def from_class(cls, node_class: type, **kwargs) -> "DANodeDescriptor":
        ...
        if not descriptor_data:
            raise ValueError(f"类 {node_class.__name__} 没有 _node_descriptor 属性，请先使用 NodeDef 装饰器")
        # 合并覆盖参数
