...
def _try_convert_to_datetime(series: pd.Series, sample_size: int = 100) -> pd.Series:
    ...
    try:
        ...
        if success_rate >= 0.8:
            ...
            logger.debug(
                f"Column '{series.name}' detected as datetime, converted successfully")
            return full_converted
    except Exception as e:
        logger.debug(
            f"Failed to convert column '{series.name}' to datetime: {e}")
...
def _postprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    '''
    对读取的 DataFrame 进行统一后处理：列名转字符串、自动检测日期列

    @param df: pd.DataFrame 要处理的DataFrame
    @return: pd.DataFrame 处理后的DataFrame
    '''
    df.columns = df.columns.astype(str)
    return _auto_detect_datetime_columns(df)
...
def detect_encoding(file_path, chunk_size=1024):
    ...
    if encoding is None or confidence < 0.5:
        logger.debug(
            "Detection confidence is too low, using default encoding 'utf-8'")
        return 'utf-8'
...
def read_csv(path: str, args: Optional[Dict] = None) -> pd.DataFrame:
    ...
    fallback_encodings = ['utf-8', 'gbk',
                          'gb2312', 'gb18030', 'utf-8-sig', 'latin1']
    ...
    raise UnicodeDecodeError(
        'utf-8', b'', 0, 1, f"Failed to read CSV file with all tried encodings: {encodings_to_try}")
...
def da_read(path: str, args: Optional[Dict] = None):
    ...
    if fun is None:
        supported = ", ".join(da_global_reader_dict.keys())
        raise ValueError(
            f"Unsupported file type: {suffix!r} (path: {path}). "
