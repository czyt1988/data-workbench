...
def peak_analysis(waveform, sampling_rate, height=None, direction=0, threshold=None, distance=None, prominence=None,
    ...
    if direction == 0 or direction == 2:
        upeak, upeak_pro = scipy.signal.find_peaks(
            udata, height, threshold, distance, prominence, width, wlen, rel_height, plateau_size)
        # 处理峰值数据
...
def wavelet_cwt(waveform, sampling_rate, scales, wavelet, sampling_period, method='conv', axis=-1):
    ...
    # 连续小波变换
    coef, freqs = pywt.cwt(waveform, scales, wavelet,
                           sampling_period, method, axis)
    return coef, freqs
...
def da_stft_analysis(waveform, sampling_rate, args: Optional[Dict] = None):
    ...
    times, frequencies, Zxx = short_time_fourier_transform(
        waveform, sampling_rate, **args)
