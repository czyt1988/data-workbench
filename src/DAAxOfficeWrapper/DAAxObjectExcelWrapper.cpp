...
bool DAAxObjectExcelWrapper::PrivateData::initialize(bool visible, bool displayAlerts)
{
	...
	if (r != S_OK && r != S_FALSE) {
		qWarning().noquote() << DAAxObjectExcelWrapper::tr("can not initialize OLE,error code:").arg((int)r);
	}
	...
	if (!mAxWorkbooks) {
		qWarning().noquote() << DAAxObjectExcelWrapper::tr("can not query subobject 'Workbooks'");
		return false;
	}
	return true;
}
