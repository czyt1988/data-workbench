...
//-----------------------------------------------------------------------------
ctkFileCompleter::ctkFileCompleter(QObject* o, bool showFiles) : QCompleter(o), CustomFileSystemModel(NULL)
{
...
ctkPathLineEditPrivate::ctkPathLineEditPrivate(ctkPathLineEdit& object)
    ...
    , Filters(QDir::AllEntries | QDir::NoDotAndDotDot | QDir::Readable | QDir::Executable)
    , HasValidInput(false)
{
}
...
// don't accept invalid path
    ...
    QObject::connect(this->BrowseButton, SIGNAL(clicked()), q, SLOT(browse()));
...
void ctkPathLineEditPrivate::createPathLineEditWidget(bool useComboBox)
{
    ...
    QObject::connect(this->LineEdit->completer()->completionModel(), SIGNAL(layoutChanged()), q, SLOT(_q_recomputeCompleterPopupSize()));
    this->LineEdit->setValidator(this->Validator);

    if (q->layout() && q->layout()->itemAt(0)) {
        delete q->layout()->itemAt(0)->widget();
    }
    qobject_cast< QHBoxLayout* >(q->layout())
        ->insertWidget(0, this->ComboBox ? qobject_cast< QWidget* >(this->ComboBox) : qobject_cast< QWidget* >(this->LineEdit));

    this->updateFilter();
    q->retrieveHistory();
    q->setCurrentPath(path);

    QObject::connect(this->LineEdit, SIGNAL(textChanged(QString)), q, SLOT(setCurrentDirectory(QString)));
    QObject::connect(this->LineEdit, SIGNAL(textChanged(QString)), q, SLOT(updateHasValidInput()));
    q->updateGeometry();
}
...
QSize ctkPathLineEditPrivate::recomputeSizeHint(QSize& sh) const
{
    ...
    if (!sh.isValid()) {
        ...
        if (this->ComboBox) {
            ...
            int arrowWidth = this->ComboBox->style()
                                 ->subControlRect(QStyle::CC_ComboBox, &option, QStyle::SC_ComboBoxArrow, this->ComboBox)
                                 .width()
                             + (this->ComboBox->hasFrame() ? 2 : 0);
            frame = 2 * (this->ComboBox->hasFrame() ? 3 : 0) + arrowWidth + 1;  // for mac style, not sure why
        } else {
            ...
            frame = 2 * frameWidth + this->LineEdit->textMargins().left() + this->LineEdit->textMargins().right()
                    + this->LineEdit->contentsMargins().left() + this->LineEdit->contentsMargins().right() + 2 * horizontalMargin;
        }
        int browseWidth = 0;
        ...
        int height  = (this->ComboBox ? this->ComboBox->minimumSizeHint() : this->LineEdit->minimumSizeHint()).height();
        sh.rwidth() = frame + textWidth + browseWidth;
        sh.rheight() = height;
    }
    return sh.expandedTo(QSize(0, 0));
}
...
void ctkPathLineEditPrivate::_q_recomputeCompleterPopupSize()
{
    ...
    int frame      = 2 * frameWidth + view->contentsMargins().left() + view->contentsMargins().right();
...
{
    Q_Q(const ctkPathLineEdit);
    return QString("ctkPathLineEdit/") + (this->SettingKey.isEmpty() ? q->objectName() : this->SettingKey);
}

//-----------------------------------------------------------------------------
ctkPathLineEdit::ctkPathLineEdit(QWidget* parentWidget)
    : QWidget(parentWidget), d_ptr(new ctkPathLineEditPrivate(*this))
{
    Q_D(ctkPathLineEdit);
    d->init();
...
//-----------------------------------------------------------------------------
ctkPathLineEdit::ctkPathLineEdit(const QString& label, const QStringList& nameFilters, Filters filters, QWidget* parentWidget)
    : QWidget(parentWidget), d_ptr(new ctkPathLineEditPrivate(*this))
{
    Q_D(ctkPathLineEdit);
    d->init();
...
void ctkPathLineEdit::updateHasValidInput()
{
    ...
    if (d->HasValidInput) {
        ...
        ctkPathLineEditPrivate::sCurrentDirectory = fileInfo.isFile() ? fileInfo.absolutePath() : fileInfo.absoluteFilePath();
...
{
    Q_D(ctkPathLineEdit);
    if (d->MinimumContentsLength == length || length < 0)
        return;

    d->MinimumContentsLength = length;

    if (d->SizeAdjustPolicy == AdjustToContents || d->SizeAdjustPolicy == AdjustToMinimumContentsLength) {
        d->SizeHint = QSize();
        d->adjustPathLineEditSize();
        this->updateGeometry();
    }
