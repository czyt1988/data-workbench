...
// -------------------------------------------------------------------------
ctkTreeComboBoxPrivate::ctkTreeComboBoxPrivate(ctkTreeComboBox& object) : q_ptr(&object)
{
...
// -------------------------------------------------------------------------
ctkTreeComboBox::ctkTreeComboBox(QWidget* _parent) : Superclass(_parent), d_ptr(new ctkTreeComboBoxPrivate(*this))
{
...
bool ctkTreeComboBox::eventFilter(QObject* object, QEvent* _event)
{
    ...
    case QEvent::MouseButtonRelease:
        ...
        if (this->view()->model()->hasChildren(index) && (index.flags() & Qt::ItemIsSelectable)
            && !this->view()->visualRect(index).contains(mouseEvent->pos())) {  // qDebug() << "Set skip on";
        ...
        if (this->view()->model()->hasChildren(index) && !(index.flags() & Qt::ItemIsSelectable)
            && !this->view()->visualRect(index).contains(mouseEvent->pos())) {  // qDebug() << "eat";
            // eat the event, don't go to the QComboBox event filters.
            res = true;
        }

        d->SendCurrentItem = this->view()->rect().contains(mouseEvent->pos()) && this->view()->currentIndex().isValid()
...
void ctkTreeComboBox::hidePopup()
{
    ...
    if (d->SkipNextHide) {  // don't hide the popup if the selected item is a parent.
...
void ctkTreeComboBox::resizePopup()
{
    ...
    }
    {
        ...
        heightMargin += marginTop + marginBottom;
#else
        QMargins m = container->contentsMargins();
        // add the frame of the container
        int marginTop, marginBottom;
        heightMargin += m.top() + m.bottom();

        // add the frame of the view
        m = this->view()->contentsMargins();
        heightMargin += m.top() + m.bottom();
#endif
    ...
    if (usePopup) {
        ...
        if (screen.width() < screen.height()) {
            ...
            if (staConTopRect.IsEmpty()) {
                ...
                (opt.direction == Qt::LeftToRight) ? listRect.setRight(screen.right()) : listRect.setLeft(screen.left());
            }
        }
#endif
