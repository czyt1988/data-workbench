...
DAPyNodeWidget::DAPyNodeWidget(QWidget* parent)
    : DAAbstractNodeWidget(parent), DA_PIMPL_CONSTRUCT
{
    ...
    try {
        for (auto it = d_ptr->mParamWidgets.begin(); it != d_ptr->mParamWidgets.end(); ++it) {
            ...
            if (value.type() == QMetaType::QString) {
#endif
                result[paramName.toStdString()] = pybind11::str(value.toString().toStdString());
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
            } else if (value.typeId() == QMetaType::Int) {
#else
            } else if (value.type() == QMetaType::Int) {
#endif
                result[paramName.toStdString()] = pybind11::int_(value.toInt());
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
            } else if (value.typeId() == QMetaType::Double) {
#else
            } else if (value.type() == QMetaType::Double) {
#endif
                result[paramName.toStdString()] = pybind11::float_(value.toDouble());
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
            } else if (value.typeId() == QMetaType::Bool) {
#else
            } else if (value.type() == QMetaType::Bool) {
#endif
                result[paramName.toStdString()] = pybind11::bool_(value.toBool());
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
                ...
                result[paramName.toStdString()] = pyList;
            }
        }
    } catch (const pybind11::error_already_set& e) {
