...
| Agent 工具/设置 | `plugins/DAAgentTools/`（20 内置工具）/ `src/APP/SettingPages/DAAgentSettingsWidget`（LLM 设置页） | 工具由 `DAAgentToolsPlugin` 注册；设置页经 `setAgentInterface` 持久化 `agent-config.json`（DAAgent 不依赖 DAGui） |
