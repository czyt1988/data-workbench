...
| Agent 工具/设置 | `plugins/DAAgentTools/`（20 内置工具）/ `src/APP/SettingPages/DAAgentSettingsWidget`（LLM 设置页） | 工具由 `DAAgentToolsPlugin` 注册；设置页经 `setAgentInterface` 持久化 `agent-config.ini`（DAAgent 不依赖 DAGui） |
| Agent 权限层 | `src/DAAgent/DAAgentPermissionManager`（权限引擎：三模式 yolo/auto/manual × 五分级决策、路径规则、会话记忆）/ `src/APP/SettingPages/DAAgentPermissionSettingsWidget`（权限设置页）/ `src/PyScripts/DAWorkbench/agent/permission_judge.py`（代码内容判定：静态规则+判官） | 权限门在 `DAAgentBridge::executeTool` 前置执法（C++ 唯一执法点）；代码判定由 Python 在 `tool_call` 前完成、裁决附 `safety` 字段下发；配置经 `get/setPermissionConfig`（`agent-permissions.json` + `agent-config.ini`）；**默认模式为全自动（yolo）**，未配置即静默放行（硬 deny 仍全模式生效）；详见 `src/DAAgent/AGENTS.md` 铁律 T16 |
