...
提供 DataFrame 数据操作纯函数，供 GUI Worker 和工作流节点调用。
...
def describe_dataframe(df: pd.DataFrame, include=None, percentiles=None) -> pd.DataFrame:
    """
    生成 DataFrame 的描述性统计摘要

    :param df: 输入 DataFrame
    :param include: 包含的数据类型，默认 None（仅数值列）
    :param percentiles: 自定义百分位数列表（如 [0.1, 0.25, 0.5, 0.75, 0.9]），默认 None
    :return: 描述性统计 DataFrame
    """
    kwargs = {"include": include}
    if percentiles is not None:
        kwargs["percentiles"] = percentiles
    return df.describe(**kwargs).copy()
