...
> **复核结论速览**：初版报告的 8 个问题经逐条对照代码**全部属实**（问题 1/2/3/5 另有影响面补充）。深度复核另发现 **8 个高严重度**（问题 9–16）、**13 个中严重度**（问题 17–29）新缺陷与一批低严重度缺口（§ 七），并整理出 **6 个待拍板决策点**（§ 九）。其中问题 9（运行期错误即杀死子进程）、10（冷启动重复注入用户消息）、12（工具执行无取消语义 + 跨会话超时误报）、13（run_code 跨会话共享命名空间）、14/15（Dock busy 守卫漏洞）为最高优先级。
...
- **整改建议**：短期按初版方案增加 `mPendingErrors[sessionId]` 缓存 + 切回重发；是否升级为"error 记录落盘 JSONL + 重放渲染"见决策点 4。
...
- **建议（复核后更新）**：**直接落 (b)**。初版"短期 (a)、中期 (b)"的建议基于 (a) 代价低的判断；复核发现 (a) 在"后台跑完自动退役"高频路径上的误清代价被低估，且 (b) 一并根治过度清除面，改动量可控（三处 + 测试），性价比反超 (a)。
...
- **整改方向**：涉及执行模型选型，**见决策点 2（§ 九）**。不论选哪个方向，12b 的"执行前存活守卫 + 孤儿结果不落盘"是低成本必做项。
...
- **整改方向**：**见决策点 3（§ 九）**（按会话命名空间隔离 vs 接受共享并文档化 vs 折中方案）。
...
- **整改建议**：① `newSession` 复用 switchSession step 1 的切离退役判断（空闲无挂起 → retire）；② 打开工程时对旧工程桥的处置**需拍板（决策点 5，§ 九）**；③ 可加桥池上限 + LRU 退役作兜底。
...
| L15 | Module :1474-1488 | `exportActiveSessions` 只导出当前 + 运行中桥的会话；绑定工程的**空闲已退役**会话不进工程 zip（本机 store 仍保留）——异机打开工程时这些会话丢失。是设计意图（D3"复制活跃会话"）还是缺口需确认 | 待确认 | 见决策点 6 |
...
### 决策点 2：工具执行模型（承接问题 12/20/23/26，并发缺陷的共同根因）
...
### 决策点 3：run_code/run_script 跨会话隔离（承接问题 13）

| 方案 | 利 | 弊 |
|------|----|----|
| (a) 按会话命名空间 + 工作区子目录 | 彻底隔离，并发分析互不污染；符合"每会话=独立笔记本"直觉 | DAPyScriptRunner 需支持多命名空间（中等改动）；da_app 预导入按会话复制有启动成本；跨会话共享数据的合法场景（两个会话分析同一批数据）需经 da_app 全局入口而非变量直传，用户心智要引导 |
| (b) 接受共享 + 文档化 | 零改动 | 并发场景正确性缺陷保留，且**静默出错**（分析结果被污染无报错）——对数据分析工作台是最坏的失败模式；"并发会话"卖点与"共享命名空间"根本矛盾 |
| (c) 折中：命名空间按会话隔离，工作区仍共享 | 隔离变量污染（最高频症状）；工作区文件天然按会话产物共存 | 文件层串扰（问题 26 TOCTOU、同名脚本互覆）仍在；两个隔离维度语义不一致需解释 |

- **推荐方向：(a)**，与决策点 2 的 (c)（run_code 限流）组合后，"每会话独立命名空间 + 全局串行执行"模型自洽；若资源紧张先落 (c) 过渡。**需要你拍板**：跨会话是否确有"共享变量"的合法需求（若有，(a) 需要设计显式共享通道）。

### 决策点 4：错误信息的持久化深度（承接问题 3）
...
- **推荐方向：(c)**，并把 L14（会话对话框加"停止"菜单）纳入同一批 UI 改造。**需要你拍板**：工程边界的产品语义（工程=会话容器，还是 merely 过滤器）。

### 决策点 6：工程保存的会话导出范围（承接 L15）
...
- **推荐方向：(b) + 数量/体积上限**（如最多 N 个最近会话），与总纲 D3"保存工程时复制活跃会话"的表述同步修订。**需要你确认** D3 原意。
...
| 12 | 工具无取消/存活语义 + 队头阻塞 | **高** | 并发放大 | 🔴/🟡 | 12b 存活守卫先落；执行模型见决策点 2 |
| 13 | run_code 跨会话共享命名空间 | **高** | 并发放大 | 🟡 | 见决策点 3 |
...
| 5 | 权限记忆跨会话失效 | 中高（安全） | e1053d1 回归 | 🟡 | 见决策点 1（推荐直接方案 b） |
| 18 | newSession/开工程桥累积 | 中高（资源） | e1053d1 遗漏 | 🔴/🟡 | newSession 补切离退役；开工程处置见决策点 5 |
| 3 | 后台错误切回丢失 | 中 | e1053d1 回归 | 🔴/🟡 | 短期 mPendingErrors；深度见决策点 4 |
...
| 25 | ${workspace} 全局漂移 | 中 | 并发放大 | 🟡 | 与决策点 1 方案 b 同批（权限状态会话化） |
