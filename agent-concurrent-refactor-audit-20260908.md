...
## 三、设计决策（原 🟡 待拍板，2026-09-08 已拍板方案 b）——复核补充
...
- **整改方向**：**已拍板（2026-09-08，决策点 3 方案 c）**：只隔离变量表（每会话一张 Python 变量表，经"当前执行会话"RAII 上下文守卫按会话查表），工作目录保持共享；跨会话数据共享走 `da_app` 全局数据入口；文件层风险由问题 26 哈希校验兜底。详见决策点 3 拍板记录。
...
- **整改建议**：C++ 执行侧改为消费判定时已读取的内容（判定载荷携带文件内容或哈希，执行前校验哈希一致），或执行时重判定。决策点 2/3 已拍板（执行队列 + 变量表隔离），本项校验随决策点 2 的执行队列改造同批落地。
...
## 九、决策点（已全部拍板，含推进方向与利弊）

> 以下 6 项无法由审计单方拍板（涉及产品语义/安全承诺/架构投入），每项给出候选方案利弊与**推荐方向**。**2026-09-08：6 项已全部经用户拍板**，各项"拍板记录"为最终结论；整改启动时间另行安排（当前为存档状态）。
...
### 决策点 3：run_code/run_script 跨会话隔离（承接问题 13）【✅ 已拍板：方案 (c)】
...
- **拍板记录（2026-09-08）**：**方案 (c) 只隔离变量表，工作目录保持共享**。跨会话故意共享数据的需求经显式通道满足——`da_app.getCore().getDataManagerInterface().getAllDataframes()` 全局数据入口（run_code 工具说明已内置此引导），不开放跨会话变量直传。落地要点：`DAPyScriptRunner` 静态单例变量表（`s_data`）改为按会话 id 查表；借助决策点 2 已定的主线程串行执行，以"当前执行会话"RAII 上下文守卫（`executeToolNow` 入口设置、Runner 读取选表）传递会话身份，不改 `DAAbstractAgentTool::execute` 公开 API；文件层残留风险（同名脚本互覆/TOCTOU）由问题 26 的"判定载荷带内容哈希、执行前校验"兜底。
...
| 13 | run_code 跨会话共享命名空间 | **高** | 并发放大 | 🔴 | **已拍板方案 (c)**：只隔离变量表（Runner 按会话查表 + 执行上下文守卫），工作目录共享，跨会话数据走 da_app 入口 |
...
| 26 | run_script TOCTOU | 中 | 并发放大 | 🔴 | 判定载荷带内容/哈希，执行前校验（随决策点 2 执行队列改造同批） |
